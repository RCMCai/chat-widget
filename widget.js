// Rapid Reply Chat Widget - Complete Final Version
(function() {
    'use strict';
    ...
    function getBusinessHoursMessage() {
        ...
        days.forEach(function(day) {
            var hours = businessHours[day];
            if (hours && !hours.closed) {
                message += day.charAt(0).toUpperCase() + day.slice(1) + ': ' + hours.start + ' - ' + hours.end + '\n';
            }
        });
    ...
    function addMessage(text, sender) {
        ...
        if (text.toLowerCase().includes('live agent') || text.toLowerCase().includes('human') || text.toLowerCase().includes('person')) {
            if (canTransferToAgent) {
                requestAgentBtn.style.display = 'block';
            } else {
                var hoursMsg = getBusinessHoursMessage();
                addMessage("I'd be happy to connect you with a live agent, but our support team is currently offline. " + hoursMsg + "In the meantime, I'll do my best to help you!", 'bot');
            }
        }
    }
