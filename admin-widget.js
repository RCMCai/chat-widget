// Rapid Reply Admin Widget - Floating Dashboard
(function() {
    'use strict';
        ...
        <div id="rr-admin-widget">
            ...
            <div id="rr-admin-panel">
                ...
                <div class="rr-admin-body">
                    <div class="rr-sessions-list" id="rr-sessions-list">
        ...
        fetch(webhookUrl + '?action=fetch-sessions&userId=' + userId)
            ...
            .then(function(data) {
                if (data.sessions) {
                    ...
                    // Show notification if new active sessions
    ...
    function renderSessions() {
        ...
        sessions.forEach(function(session) {
            ...
            div.innerHTML = `
                <div class="rr-session-email">${session.clientEmail || 'Unknown'}</div>
    ...
    function renderChat() {
        ...
        chatArea.innerHTML = `
            <div class="rr-chat-header">
                <div style="font-weight: 600; font-size: 16px;">${selectedSession.clientEmail || 'Client'}</div>
                <div style="font-size: 13px; color: #6b7280; margin-top: 2px;">${selectedSession.clientPhone || 'No phone'}</div>
        ...
        if (selectedSession.messages && selectedSession.messages.length > 0) {
            selectedSession.messages.forEach(function(msg) {
                ...
                var label = msg.sender === 'agent' ? 'YOU' : msg.sender === 'bot' ? 'AI' : 'CUSTOMER';
    ...
    pollInterval = setInterval(function() {
        if (adminPanel.classList.contains('open')) {
            ...
            // Still check for new sessions even when closed
            fetch(webhookUrl + '?action=fetch-sessions&userId=' + userId)
                .then(function(response) { return response.json(); })
                .then(function(data) {
                    if (data.sessions) {
